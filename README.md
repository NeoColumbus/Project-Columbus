...
Run `pnpm test:field-api`, `pnpm test:proof`, and `pnpm test:launch` (install Chromium once with `pnpm exec playwright install chromium`). Set `PUBLIC_ROOT=dist/pages` to run browser checks against the deployment artifact. The PR workflow runs these checks with no production writes. QR assets are not regenerated by this pass.
