...
const outDir = path.join(root, "print-orders", "library-sticker-test-drop-001");
const svgDir = path.join(outDir, "source-svg");
const pdfDir = path.join(outDir, "individual-pdfs");
...
function qrFor(poster) {
  ...
  const source = `sticker-${number}`;
  const asset = `${poster.id}-sticker`;
  const tmp = path.join(os.tmpdir(), `full-city-print-test-${source}.svg`);
...
function renderSticker(poster, qr) {
  ...
  <title id="title">Drop 001 ${escapeXml(poster.title)} Printer Test Sticker</title>
  <desc id="desc">A letter-size printer-test sticker with a distinct QR code linking to ${escapeXml(qr.url)}.</desc>
  <defs>
  ...
  <text x="150" y="194" fill="#050606" font-family="Arial Black, Impact, sans-serif" font-size="74" letter-spacing="5">DROP 001 / PRINTER TEST / QR</text>
  ...
  <g transform="translate(455 450)">
    ...
    <text x="72" y="2278" fill="#9D4732" font-family="Arial Black, Impact, sans-serif" font-size="26" letter-spacing="4">SOURCE ${escapeXml(qr.source.toUpperCase())} / PRINT TEST</text>
...
async function pdfPageFromPng(pdf, pngBuffer) {
...
async function writeSinglePdf(pngBuffer, outputPath) {
  const pdf = await PDFDocument.create();
  await pdfPageFromPng(pdf, pngBuffer);
...
async function main() {
  ...
  const rows = [["file", "source", "asset", "url"]];

  for (const poster of posters) {
    const qr = qrFor(poster);
    const stem = `${poster.id}-printer-test-qr`;
    const svgText = renderSticker(poster, qr);
    const svgPath = path.join(svgDir, `${stem}.svg`);
    const pdfPath = path.join(pdfDir, `${stem}.pdf`);

    fs.writeFileSync(svgPath, svgText);
    const pngBuffer = await pngBufferFromSvg(svgText);
    await writeSinglePdf(pngBuffer, pdfPath);
    await pdfPageFromPng(combined, pngBuffer);

    rows.push([`${stem}.pdf`, qr.source, qr.asset, qr.url]);
    console.log(`wrote ${path.relative(root, pdfPath)}`);
  }

  fs.writeFileSync(path.join(outDir, "full-city-sticker-print-test-all.pdf"), await combined.save());
  fs.writeFileSync(
    path.join(outDir, "scan-paths.csv"),
    `${rows.map((row) => row.map((value) => `"${String(value).replace(/"/g, '""')}"`).join(",")).join("\n")}\n`
  );
  fs.writeFileSync(
    path.join(outDir, "README.md"),
    [
      "# Library Sticker Print Test - Drop 001",
      "",
      "Use `individual-pdfs/` for the print order if the library wants one file per sticker.",
      "Use `full-city-sticker-print-test-all.pdf` if the library wants one multi-page PDF.",
      "",
      "Every sticker has a distinct QR code with `drop=001`, `asset`, `source`, `kind`, `break`, and `line` parameters.",
      "Scan each printed page before ordering the full run.",
      "",
      "The source SVGs are in `source-svg/` for future edits; do not upload them unless the printer explicitly accepts SVG."
    ].join("\n")
  );

  console.log(`wrote ${path.relative(root, path.join(outDir, "full-city-sticker-print-test-all.pdf"))}`);
